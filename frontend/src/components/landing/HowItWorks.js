      ...
      <div className="max-w-7xl mx-auto px-8 relative z-10">
        <Reveal>
          <div className="text-center mb-24">
            ...
            <p className="text-muted-foreground/40 font-serif italic text-lg max-w-2xl mx-auto">
        ...
        <StaggerContainer className="grid md:grid-cols-4 gap-12 relative">
          ...
          {steps.map((item, i) => (
            <motion.div key={item.step} variants={staggerChild} className="flex flex-col items-center text-center group">
              <motion.div
                whileHover={{ scale: 1.1, borderColor: "hsl(var(--primary))", boxShadow: "0 0 30px hsla(var(--primary), 0.2)" }}
                className="size-28 rounded-full bg-primary/5 backdrop-blur-xl border border-primary/20 flex items-center justify-center mb-8 transition-all duration-500 relative z-10"
              >
                ...
                <div className="absolute -bottom-2 -right-2 size-8 rounded-full bg-primary flex items-center justify-center shadow-lg">
                   <span className="text-[10px] font-mono font-black text-primary-foreground">{i+1}</span>
                </div>
              </motion.div>
              ...
              <h4 className="text-foreground font-serif font-bold mb-3 text-xl group-hover:text-primary transition-colors">{item.title}</h4>
              <p className="text-[11px] text-muted-foreground/30 font-serif leading-relaxed italic">{item.desc}</p>
            </motion.div>
          ))}
        </StaggerContainer>
      </div>
